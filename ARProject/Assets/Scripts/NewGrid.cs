...
public class NewGrid : MonoBehaviour
{
    ...
    public List<ToolInfo> inventory;
    public List<ToolInfo> items = new List<ToolInfo>();
    ...
    void Start()
    {
        items = GameDataBase.Inst.tools;
        count = items.Count;
        // 임의의 데이터가 생성해서 gird에 추가시켜둔다.
        // ItemCellData 는 IReuseCellData 상속받아서 구현된 데이터 클래스다.
        for (int i = 0; i < count; ++i)
        {
            Toolcelldata cell = new Toolcelldata();
            cell.ImgName = items[i].toolName;
            cell.cost = items[i].toolCount;
            cell.damage = items[i].toolType;
