...
public class GameDataBase : Singleton<GameDataBase>
{
    ...
    public void Ds_InventoryInsert(int DBnum)
    {
        ...
        {
            if (card.cardInventoryNum.Equals(DBnum))
            {
                int Num = cards_Inventory.Count;
                int Cost = card.cardCost;
                int Damage = card.cardDamage;
                string Name = card.cardName;
                Element element = card.cardElement;
    ...
    public string CardInfostring(int DBcardnumber)
    {
        string Info = null;
        IDbConnection dbconn;
        dbconn = (IDbConnection)new SqliteConnection(conn);
        ...
        IDbCommand dbcmd = dbconn.CreateCommand();
        string sqlQuery = "SELECT * FROM CardInfo WHERE CardNumber=\"" + DBcardnumber + "\"";
        dbcmd.CommandText = sqlQuery;
        IDataReader reader = dbcmd.ExecuteReader();
        while (reader.Read())
        {
    ...
    public void DS_ToolSetting()
    {
        ...
        IDbCommand dbcmd = dbconn.CreateCommand();
        string sqlQuery = "SELECT * FROM toolInventory";
        dbcmd.CommandText = sqlQuery;
        IDataReader reader = dbcmd.ExecuteReader();
        while (reader.Read())
        {
            ...
            tools.Add(new ToolInfo(Name, count, type));
        }
        reader.Close();
        reader = null;
        dbcmd.Dispose();
    ...
    public void Ds_ToolInsert(String Toolname,int count)
    {
        IDbConnection dbconn;
        dbconn = (IDbConnection)new SqliteConnection(conn);
        dbconn.Open();
        IDbCommand dbcmd = dbconn.CreateCommand();
        string sqlQuery = string.Format("update ToolInventory set Count = {0} where Name={1};", count, Toolname);
        dbcmd.CommandText = sqlQuery;
        IDataReader reader = dbcmd.ExecuteReader();
        reader.Close();
        reader = null;
